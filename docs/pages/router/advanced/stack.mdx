...
> `dismissTo` was added in Expo Router `4.0.8`. It operates similar to the `navigation` function in Expo Router v3

Dismisses screens in the current `<Stack />` until the specified `Href` is reached. If the `Href` is not present in the history, a `PUSH` action will be performed instead.

For example, the given a history of `/one`, `/two`, `/three`, where `/three` is the current route. The action `router.dismissTo('/one')` will cause the history to go back twice, while `router.dismissTo('/four')` will `PUSH` the history forwards to the `/four` route.
...
export default function Settings() {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      /* @info Trigger the handler method on a touchable or button component. */
      <Button title="Go to first screen" onPress={handleDismissTo} />
      /* @end */
    </View>
  );
}
